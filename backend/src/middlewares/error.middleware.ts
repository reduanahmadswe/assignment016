...
export const errorHandler = (
  ...
  // If this is an AppError (custom error with explicit statusCode), use its message as-is
  // Don't override custom error messages
  const isCustomError = err.statusCode && err.statusCode >= 400 && err.statusCode < 500;

  // Only apply generic error handling if it's NOT a custom error
  if (!isCustomError) {
