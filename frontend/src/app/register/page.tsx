...
export default function RegisterPage() {
  ...
  // No longer needed for redirect-based auth
  ...
  // Handle Google Sign-In button click for redirect flow
  const handleGoogleSignIn = () => {
    // Store that this is a registration flow
    sessionStorage.setItem('oauth_redirect', '/dashboard');
    sessionStorage.setItem('oauth_type', 'register');
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-5">
                ...
                {process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID && (
                  <button
                    ...
                    disabled={isLoading}
                    className="w-full h-[50px] flex items-center justify-center gap-3 px-4 bg-white text-gray-700 font-semibold rounded-xl border-2 border-gray-300 hover:border-blue-500 hover:bg-gray-50 transition-all disabled:opacity-50 disabled:cursor-not-allowed mt-4"
                  >
